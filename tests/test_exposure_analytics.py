    ...
    def test_build_exposure_payload(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(os.path.join(td, "trader_data.json"), "w", encoding="utf-8") as f:
                json.dump(
                    {
                        "account": {"total_account_value": 250.0},
                        "positions": {"BTC": {"value_usd": 120.0, "quantity": 0.01}, "ETH": {"value_usd": 0.0}},
                    },
                    f,
                )
            with open(os.path.join(td, "stocks", "alpaca_status.json"), "w", encoding="utf-8") as f:
                json.dump({"equity": 1000.0, "raw_positions": [{"symbol": "AAPL", "market_value": "80.5", "qty": "1"}]}, f)
            with open(os.path.join(td, "stocks", "stock_trader_status.json"), "w", encoding="utf-8") as f:
                json.dump({"account_value_usd": 1000.0}, f)
            ...
            with open(os.path.join(td, "forex", "forex_trader_status.json"), "w", encoding="utf-8") as f:
                json.dump({"account_value_usd": 500.0}, f)
            ...
            self.assertEqual(float(out.get("account_value_by_market_usd", {}).get("stocks", 0.0) or 0.0), 1000.0)
            self.assertEqual(float(out.get("total_account_value_usd", 0.0) or 0.0), 1750.0)
            self.assertIn("pct_of_market_account", out["top_positions"][0])
